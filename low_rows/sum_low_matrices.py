...
def generateChunks(k: int, max_sum=10):
    final = []
    base = np.eye(k)[::-1]
    bases = [base * i for i in range(0, max_sum + 1)]

    fill_positions = [(i, j) for i in range(k) for j in range(k) if i + j < k - 1]
    combos = list(product(range(max_sum + 1), repeat=len(fill_positions)))
    ...
    for b in bases:
        for c in combos:
            ...
            for idx, (i, j) in enumerate(fill_positions):
                mat[i, j] = c[idx]
            mat = mat + b
            if np.sum(mat) <= max_sum:
                final.append(mat)
    return final

def all_kxk_matrices(k: int, max_sum=10):
    vals = range(max_sum + 1)
...
def gen_giants(A, B, C, k):
    ...
    for a in A:
        for b in B:
            for c in C:
                total = np.sum(a) + np.sum(b) + np.sum(c)
                if (np.any(a) or np.any(b) or np.any(c)):
